...
CGI::WPM::SequentialFile - Perl module that interfaces to a common text file format
...
package CGI::WPM::SequentialFile;
require 5.004;
...
=head1 SYNOPSIS
	...
	my $field_defin_file = CGI::WPM::SequentialFile->new( "GB_Questions.txt" );
	my $message_file = CGI::WPM::SequentialFile->new( 
	...
	my $user_input = CGI::MultiValuedHash->new( $case_insensitive, $query_string );
...
my $KEY_CASE_INSE  = 'case_inse';   # are MVH keys case insensitive?
...
Record data taken from a file is returned as a list of CGI::MultiValuedHash
(MVH) objects, one object for each record.  The keys in the MVH are the field
names, and the list of values associated with each MVH key are the values of the
field.  Record data to be stored in a file must likewise be provided as a list of
MVH objects, or a list of HASH refs.  MVHs are used because they simplify the
manipulation of hashes whose keys may have one or several values (see the MVH
...
This function creates a new CGI::WPM::SequentialFile object and returns it.  The
...
properties recognized by CGI::WPM::SequentialFile are set in the clone; other 
...
the MVH objects that read records are returned in.  Any MVHs with this property
